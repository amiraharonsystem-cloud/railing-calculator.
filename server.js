...
def get_report():
    ...
    # שימוש בקישור ייצוא ציבורי שעוקף חסימות API
    url = f"https://docs.google.com/spreadsheets/d/{SPREADSHEET_ID}/gviz/tq?tqx=out:csv&sheet={target_date}"
    
    try:
        # הוספת User-Agent כדי להיראות כמו דפדפן רגיל
        df = pd.read_csv(url, storage_options={'User-Agent': 'Mozilla/5.0'})
        df = df.fillna('')
        
        # חיפוש "אמיר אהרון" בנתונים
        ...
        return f"<div dir='rtl'><h1>שגיאה בגישה לגיליון</h1><p>פרטים: {str(e)}</p><a href='/'>חזור</a></div>", 500
